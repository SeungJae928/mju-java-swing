...
class MyFrame extends JFrame {
	...
	ArrayList<Thing> figures = new ArrayList<>();
	...
	public MyFrame() {
		...
		ActionListener lis = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				} else if (((AbstractButton) p).getText().equals("지우기")) {
					figures.clear();
					repaint();
				}
			}
	...
	public void processMouseEvent(MouseEvent e) {
		...
		case MouseEvent.MOUSE_CLICKED:
			if(e.getButton() == MouseEvent.BUTTON3) {
				for(Thing figure : figures) {
					if (figure.contain(x1, y1)) {
						type = 4;
						g = figure;
						System.out.println("Selected Object : " + figure);
						break;
					}
				}
			}
		...
		case MouseEvent.MOUSE_RELEASED:
			...
			if (type == 0) {
				...
				for(int i = 0; i < remove.size(); i++) {
                    for(int j1 = 0; j1 < figures.size(); j1++) {
                    	if(remove.get(i) == figures.get(j1)) {
                    		figures.remove(figures.get(j1));
                    	}
                    }
				}
				repaint();
